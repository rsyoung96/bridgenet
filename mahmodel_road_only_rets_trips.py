...
def compute_tt_vmt(damaged_graph, demand):
	...
	newG, new_demand, orig_demand = it.assign()
	...
	'''
	return travel_time, vmt, new_demand, orig_demand
...
def add_superdistrict_centroids(G):
	...
	for row in sd_table:
		...
		G.add_edge(str(1000000 + i), str(row[1]), capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1,
				   distance=1, t_a=1, t_0=1, flow=0,
				   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
		G.add_edge(str(1000000 + i), str(row[2]), capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1,
				   distance=1, t_a=1, t_0=1, flow=0,
				   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
		G.add_edge(str(row[3]), str(1000000 + i), capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1,
				   distance=1, t_a=1, t_0=1, flow=0,
				   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
		G.add_edge(str(row[4]), str(1000000 + i), capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1,
				   distance=1, t_a=1, t_0=1, flow=0,
				   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	...
	G.add_node('oak')
	G.add_node('sfo')
	G.add_edge('sf', '1000001', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('sf', '1000002', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('sf', '1000003', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('sf', '1000004', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('sf', '1000005', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('1000018', 'oak', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('1000019', 'oak', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('1000020', 'oak', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1,
			   t_a=1, t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('6564', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('6563', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('6555', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('9591', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('6550', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
	G.add_edge('6599', 'sfo', capacity_0=100000, capacity=100000, lanes=1, bridges=[], distance_0=1, distance=1, t_a=1,
			   t_0=1, flow=0,
			   dailyvolume=1)  # capacity in vehicles over all lanes, travel time in seconds, length in miles, flow in
...
def damage_bridges(scenario, master_dict):
	...
	for site in master_dict.keys():  # 1-1889 in Matlab indices (start at 1)
		...
		prob_at_least_ext = norm.cdf((1 / float(beta)) * (lnSa - math.log(master_dict[site]['ext_lnSa'])), 0,
									 1)  # want to do moderate damage state instead of extensive damage state as we did here, then just change the key name here (see master_dict description)
...
def compute_damage(scenario, master_dict, index):
	...
	damaged_bridges_internal, damaged_bridges_new, num_damaged_bridges = damage_bridges(scenario,
																						master_dict)  # e.g., [1, 89, 598] #num_bridges_out is highway bridges only
...
def measure_performance(damaged_graph, demand):
	...
	travel_time, vmt, new_demand, orig_demand = compute_tt_vmt(damaged_graph, demand)
	# print 'inside measure performance', new_demand, orig_demand
	return flow, shortest_paths, travel_time, vmt, new_demand, orig_demand


def compute_road_performance(G, damaged_bridges_internal, demand, no_damage_travel_time, no_damage_vmt,
							 no_damage_demand, no_damage_full_demand, no_damage_flow, no_damage_shortest_path,
							 master_dict, index):
	...
	if len(damaged_bridges_internal) > 0:
		...
		flow, shortest_paths, travel_time, vmt, new_demand, orig_demand = measure_performance(G, demand)
		# print 'after measure performance', new_demand, orig_demand
		G = util.clean_up_graph(
			G)  # absolutely critical. otherwise, damage from scenario gets added to damage from previous scenarios!
		...
		vmt = no_damage_vmt
		road_bridges_out = 0
		new_demand = no_damage_demand
		orig_demand = no_damage_full_demand
	# print 'inside compute road performance:', new_demand, orig_demand

	print 'total network performance calculation time: ', time.time() - start_time
	return index, road_bridges_out, flow, shortest_paths, travel_time, vmt, new_demand, orig_demand
...
def save_results(bridge_array_internal, bridge_array_new, travel_index_times, numeps, seed):
	util.write_2dlist(
		'output/' + time.strftime("%Y%m%d") + '_bridges_flow_path_tt_vmt_bridges_allBridges_roadonly_' + str(
			numeps) + 'eps_extensive_seed' + str(seed) + '.txt', travel_index_times)
	with open(
			'output/' + time.strftime("%Y%m%d") + '_' + str(numeps) + 'sets_damagedBridgesInternal_roadonly_seed' + str(
					seed) + '.pkl', 'wb') as f:
		pickle.dump(bridge_array_internal, f)
	with open('output/' + time.strftime("%Y%m%d") + '_' + str(numeps) + 'sets_damagedBridgesNewID_roadonly_seed' + str(
			seed) + '.pkl', 'wb') as f:
...
def save_results_0(bridge_array_internal, bridge_array_new, numeps, seed):
	with open(
			'output/' + time.strftime("%Y%m%d") + '_' + str(numeps) + 'sets_damagedBridgesInternal_roadonly_eed' + str(
					seed) + 'temp.pkl', 'wb') as f:
		pickle.dump(bridge_array_internal, f)
	with open('output/' + time.strftime("%Y%m%d") + '_' + str(numeps) + 'sets_damagedBridgesNewID_roadonly_seed' + str(
			seed) + 'temp.pkl', 'wb') as f:
		pickle.dump(bridge_array_new, f)
...
def main():
	...
	print 'You are considering %d different damage maps (%d per ground-motion intensity map).' % (
	int(len(targets)), int(sets))
	...
	jobs = []
	for i in targets:
		jobs.append(job_server.submit(compute_damage, (lnsas[i % len(lnsas)], master_dict, targets[i],),
	...
	for i in targets:
		...
		None, bridge_array_internal[i], demand, no_damage_travel_time, no_damage_vmt, no_damage_demand,
		no_damage_full_demand, no_damage_flow, no_damage_shortest_path, master_dict, targets[i],),
									  modules=('networkx', 'time', 'pickle', 'pdb', 'util', 'random', 'math', 'ita',),
									  depfuncs=(
									  get_graph, add_superdistrict_centroids, damage_bridges, damage_highway_network,
									  measure_performance, compute_flow, compute_shortest_paths,
	...
	i = 0
	for job in jobs:
		(index, road_bridges_out, flow, shortest_paths, travel_time, vmt, new_demand, orig_demand) = job()
		...
		# print 'after job completion', new_demand, orig_demand
		travel_index_times.append((index, road_bridges_out, flow, shortest_paths, travel_time, vmt, new_demand,
								   orig_demand, road_bridges_out / float(num_of_interest_bridges),
								   len(bridge_array_new[i]) / float(num_of_total_bridges),
								   magnitudes[index % len(magnitudes)]))

		# print new_demand, orig_demand
		# GB modification -- get output of interest in csv file
		csvrow = [s, retfile, no_damage_travel_time, travel_time, no_damage_vmt, vmt, new_demand, no_damage_demand,
				  orig_demand, road_bridges_out, bridge_array_new[i]]
		with open('GB_trip_outputs_50rets/traffic_output.csv', 'a') as csvfile:
			wr = csv.writer(csvfile, delimiter=',')
			wr.writerow(csvrow)

		# save as you go
		if i % len(lnsas) == 0:
			save_results(bridge_array_internal, bridge_array_new, travel_index_times, int((i + 1) / float(len(lnsas))),
						 seed_num)
		i += 1

	# save an extra time at the very end
	save_results(bridge_array_internal, bridge_array_new, travel_index_times, int((i + 1) / float(len(lnsas))),
				 seed_num)  # save again when totally done
