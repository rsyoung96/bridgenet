    ...
    for i in range(4): #do 4 iterations
      for origin in self.demand.keys(): #origin is an actual A (one end of an actual edge in the graph)
        ...
        for destination in self.demand[origin].keys(): #actual A or B (one end of an actual edge in the graph)
          ...
          total_flow += od_flow
          if len(path_list) == 0:
            print 'no path between', origin, 'and', destination # ADDED BY RY 6-9-18
            total_flow_impassable += od_flow
          ...
          for index in range(0, len(path_list) - 1):
            ...
            if self.G.is_multigraph():
              ...
              try:
                ...
                pdb.set_trace()
    return self.G, total_flow, total_flow_impassable
...
def test():
  ...
  demand = {}
  demand[1] = {}
  demand[1][2] = 8000/0.053 #divide by 0.053 since that is what we multiply by above

  #call ita
  it = ITA(G,demand)
  newG, total_flow, missing_flow = it.assign()
  print newG
  print 'total flow:', total_flow
  print 'missing flow:', missing_flow
  print 'proportion of trips lost:', (missing_flow/total_flow)
