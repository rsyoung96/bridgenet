...
#Modified by: Gitanjali Bhattacharjee
#Last modified: 5 June 2018

#import some relevant Python packages
import pickle, random, pdb, time, networkx, pp
...
def damage_bridges(scenario, master_dict_ret):
	...
	for site in master_dict_ret.keys(): #1-1889 in Matlab indices (start at 1)
		lnSa = scenario[master_dict_ret[site]['new_id'] - 1]
		prob_at_least_ext = norm.cdf((1/float(beta)) * (lnSa - math.log(master_dict_ret[site]['ext_lnSa'])), 0, 1) #want to do moderate damage state instead of extensive damage state as we did here, then just change the key name here (see master_dict_ret description)
		U = random.uniform(0, 1)
		if U <= prob_at_least_ext:
			damaged_bridges_new.append(master_dict_ret[site]['new_id']) #1-1743
			damaged_bridges_internal.append(site) #1-1889
	num_damaged_bridges = sum([1 for i in damaged_bridges_new if i <= len(master_dict_ret.keys())])
	print 'num_damaged_bridges = '
	print num_damaged_bridges
	print 'damaged bridges internal = '
	print damaged_bridges_internal
	return damaged_bridges_internal, damaged_bridges_new, num_damaged_bridges

def compute_damage(scenario, master_dict_ret, index):
	'''goes from ground-motion intensity map to damage map '''
	#figure out component damage for each ground-motion intensity map
	damaged_bridges_internal, damaged_bridges_new, num_damaged_bridges = damage_bridges(scenario, master_dict_ret) #e.g., [1, 89, 598] #num_bridges_out is highway bridges only
	return index, damaged_bridges_internal, damaged_bridges_new, num_damaged_bridges

def damage_highway_network(damaged_bridges_internal, G, master_dict_ret, index):
	'''damaged bridges is a list of the original ids (1-1889, not the new ids 1-1743!!!!!!!) '''
	biggest_id_of_interest = max([int(k) for k in master_dict_ret.keys()])
	...
	for site in damaged_bridges_internal:
		if int(site) <= 1889: #in original ids, not new ones since that is what is in the damaged bridges list
			affected_edges = master_dict_ret[site]['a_b_pairs_direct'] + master_dict_ret[site]['a_b_pairs_indirect']
...
## GB ADDITION
#tt_increase = travel_time - no_damage_travel_time
#print tt_increase
